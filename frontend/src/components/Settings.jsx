...
export default function Settings() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch credits      const creditsRes = await axios.get(`${API_BASE_URL}/credits/balance`, {        headers: { Authorization: `Bearer ${token}` },      });
      setCredits(creditsRes.data);
      // Fetch pricing      const pricingRes = await axios.get(`${API_BASE_URL}/credits/pricing`);
      setPricing(pricingRes.data);
      // Fetch transactions      const transactionsRes = await axios.get(`${API_BASE_URL}/credits/transactions`, {        headers: { Authorization: `Bearer ${token}` },      });
      setTransactions(transactionsRes.data.transactions);
      
      // Determine active plan based on last purchase transaction
      ...
      if (purchaseTransactions.length > 0) {
        ...
        // Match with pricing plans
        const matchedPlan = pricingRes.data.find(p => 
  ...
  const handlePaymentSuccess = (paymentData) => {
    // Refresh credits and close modal    fetchData();
              ...
              >
                ...
                <div className="plan-details">
                  ...
                  <div className="plan-detail">
                    ...
                    <Check size={16} />{" "}
                    <span>
                      {plan.isFree ? "Perfect for testing" : `$${plan.pricePerCredit.toFixed(3)} per credit`}
                    </span>{" "}
                  </div>{" "}
                  {plan.savings && (
                    <div className="plan-detail highlight">
                      {" "}
                      <Check size={16} /> <span>Save {plan.savings}</span>{" "}
                    </div>
                  )}{" "}
                  <div className="plan-detail">
                    {" "}
                  ...
                  <div className="plan-detail">
                    {" "}
                    <Check size={16} /> <span>Never expires</span>{" "}
                  </div>{" "}
                </div>{" "}
                {!plan.isFree && (
                  <button
              ...
              {transactions.map((txn) => (
                <div key={txn._id} className="transaction-row">
                  ...
                  <div className="transaction-icon">
                    {" "}
                    {txn.type === "purchase" && (
                      <CreditCard size={20} className="icon-purchase" />
                    )}{" "}
                    {txn.type === "deduction" && (
                      <Zap size={20} className="icon-deduction" />
                    )}{" "}
                    {txn.type === "bonus" && (
                      <TrendingUp size={20} className="icon-bonus" />
                    )}{" "}
                    {txn.type === "refund" && (
                      <DollarSign size={20} className="icon-refund" />
                    )}{" "}
                  </div>{" "}
                  ...
                  <div className="transaction-amount">
                    {" "}
                    <span
                      className={`amount ${txn.type === "purchase" || txn.type === "bonus" || txn.type === "refund" ? "positive" : "negative"}`}
                    >
                      {" "}
                      {txn.type === "purchase" ||
                      txn.type === "bonus" ||
