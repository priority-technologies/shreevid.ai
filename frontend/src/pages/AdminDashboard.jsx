...
import { 
  Menu, X, LogOut, LayoutDashboard, Users, CreditCard, 
  Zap, DollarSign, Activity, Search, Plus, Trash2, 
  RefreshCw, Eye, AlertCircle, Edit2 
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_BASE_URL}/admin/users?search=${searchTerm}`, getAuthHeaders());
  ...
  const fetchTransactions = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_BASE_URL}/admin/transactions`, getAuthHeaders());
  ...
  const fetchProjects = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_BASE_URL}/admin/projects`, getAuthHeaders());
  ...
  const fetchApiUsage = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_BASE_URL}/admin/api-usage/summary`, getAuthHeaders());
  ...
  const handleAddCredits = async (userId) => {
    ...
    const reason = prompt('Reason for adding credits:') || 'Admin credit addition';
  ...
  const handleDeductCredits = async (userId) => {
    ...
    const reason = prompt('Reason for deducting credits:') || 'Admin credit deduction';
  ...
  const handleSaveUser = (updatedUser) => {
    ...
    setUsers(users.map(u => u._id === updatedUser.id ? { ...u, ...updatedUser } : u));
  };
              ...
              <div className="stats-grid">
                ...
                <div className="stat-card">
                  ...
                  <div className="stat-content">
                    ...
                    <p className="stat-sub">Completed: {stats.projects.completed}</p>
                  </div>
                </div>
                ...
                <div className="stat-card">
                  ...
                  <div className="stat-content">
                    ...
                    <h3 className="stat-value">₹{stats.revenue.total.toLocaleString()}</h3>
                  </div>
                </div>
                ...
                <div className="stat-card">
                  ...
                  <div className="stat-content">
                    ...
                    <h3 className="stat-value">{stats.credits.totalCreditsAvailable.toLocaleString()}</h3>
                    <p className="stat-sub">Issued: {stats.credits.totalCreditsIssued.toLocaleString()}</p>
                  </div>
                </div>
              </div>
                ...
                <div className="activity-list">
                  {stats.users.recent.map((u) => (
                    <div key={u._id} className="activity-item">
                      ...
                      <div>
                        <p className="activity-title">{u.firstName} {u.lastName}</p>
                  ...
                  <tbody>
                    {users.map((u) => (
                      <tr key={u._id}>
                        ...
                        <td>{u.email}</td>
                        <td><span className="badge badge-success">{u.credits}</span></td>
                        <td>{u.totalCreditsSpent}</td>
                        <td>{new Date(u.createdAt).toLocaleDateString()}</td>
                        <td>
                          <div className="action-buttons">
                            <button onClick={() => handleEditUser(u)} className="btn-icon btn-primary" title="Edit User">
                              <Edit2 size={16} />
                            </button>
                            <button onClick={() => handleAddCredits(u._id)} className="btn-icon btn-success" title="Add Credits">
                              <Plus size={16} />
                            </button>
                            <button onClick={() => handleDeductCredits(u._id)} className="btn-icon btn-warning" title="Deduct Credits">
                              <Zap size={16} />
                            </button>
                            <button onClick={() => handleDeleteUser(u._id)} className="btn-icon btn-danger" title="Delete User">
                              <Trash2 size={16} />
                            </button>
                          </div>
                        </td>
                  ...
                  <tbody>
                    {transactions.map((t) => (
                      <tr key={t._id}>
                        ...
                        <td>₹{t.amountPaid || t.amount}</td>
                        <td>{t.balanceAfter}</td>
                        <td><span className={`badge badge-${t.paymentStatus === 'completed' ? 'success' : 'warning'}`}>{t.paymentStatus}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                  ...
                  <tbody>
                    {projects.map((p) => (
                      <tr key={p._id}>
                        ...
                        <td><span className={`badge badge-${p.videoStatus === 'completed' ? 'success' : p.videoStatus === 'failed' ? 'danger' : 'warning'}`}>{p.videoStatus}</span></td>
                        <td>{p.duration}s</td>
                        <td>{p.costCredits || 0}</td>
                        <td>
                          <button onClick={() => handleViewProject(p)} className="btn-icon btn-info" title="View Project">
                            <Eye size={16} />
                          </button>
                        </td>
                      </tr>
              ...
              <div className="api-cards">
                <div className="api-card">
                  ...
                  <div className="api-details">
                    <p>Total Projects: <strong>{apiUsage.totalProjects}</strong></p>
                    <p>Model: Gen-4 Turbo</p>
                  </div>
                  <button className="btn-secondary">View Usage Details</button>
                </div>
                ...
                <div className="api-card">
                  ...
                  <div className="api-details">
                    <p>Total Videos: <strong>{apiUsage.totalProjects}</strong></p>
              ...
              <div className="summary-card">
                ...
                <div className="summary-stats">
                  <div className="summary-item">
                    <p>Total Users</p>
                    <h2>{apiUsage.totalUsers}</h2>
                  </div>
                  <div className="summary-item">
                    <p>Total Projects</p>
                    <h2>{apiUsage.totalProjects}</h2>
                  </div>
                  <div className="summary-item">
                    <p>Total Revenue</p>
                    <h2>₹{apiUsage.totalRevenue}</h2>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="project-details">
                  ...
                  <div className="detail-row">
                    <label>Status</label>
                    <p>
                      <span className={`badge badge-${selectedProject.videoStatus === 'completed' ? 'success' : selectedProject.videoStatus === 'failed' ? 'danger' : 'warning'}`}>
                  ...
                  {selectedProject.imageUrl && (
                    <div className="detail-row">
                      ...
                      <img src={selectedProject.imageUrl} alt="Project" className="project-image" />
                    </div>
                  )}
                  {selectedProject.videoUrl && (
                    <div className="detail-row">
                      <label>Video URL</label>
                      <a href={selectedProject.videoUrl} target="_blank" rel="noopener noreferrer" className="link">
                        View Video
                      </a>
                    </div>
                  )}
